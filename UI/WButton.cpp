...
void WButton::set_text(const std::string& newText) {
  ...
  if (textbox) {
    ...
    textbox->update_position((size - textbox->size) / 2);
  }
}

void WButton::post_init() {
  UIElement::post_init();
  auto textbox = uiSystem->create_widget<WTextbox>(
      shared_from_this(), Binding<std::string>(), size, sf::Vector2i{0, 0}, "ButtonText");
